...
export class LoginComponent implements OnInit {
  ...
  login() {
    if (this.formLogin.valid) {
      ...
      this.citoyenService.login(this.email, this.password).subscribe((data) => {
        this.citoyen = data;
        if (this.citoyen != null) {
          this.service
            .findPermisByCourrielAndPwd(
              this.citoyen.courriel,
              this.citoyen.password
            )
            .subscribe((permis) => {
              sessionStorage.setItem('courriel', this.email);
              this.formLogin.reset();
              this.router.navigateByUrl('/dashboard', { state: permis });
            });
        } else {
